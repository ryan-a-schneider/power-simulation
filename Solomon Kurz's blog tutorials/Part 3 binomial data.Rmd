...
```{r setup}
pacman::p_load(tidyverse,brms,report,parameters,bayestestR, insight, see, tidybayes)
```
...
The first step is to **generate data you think will resemble your experimental data**. This example is the probability that someone will hit a ball in a baseball game. IRL, the batting average is around .25, so that's where we will start.

Here, he is producing a simple binomial distribution of the proportion of batters that hit a ball.
...
Now you need to model that data with a logistic regression to get estimates on the proportion (i.e., the parameter).
...
tidy_posterior=function(model){
  output=bayestestR::describe_posterior(model, ci_method="hdi", ci=.95) %>% 
...
*That was the process for doing it once. O.k., on to iterative simulations; now we want to know what happens when we generate 100 sets of data.*

This is important because all samples are probabilistic and we never want to interpret the results of just one.
